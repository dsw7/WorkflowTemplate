...
def lint(session):
    command = 'pylint {} {} --output-format=colorized --exit-zero --rcfile {}'.format(
        PATH_TO_PROJECT, path.join(PATH_TO_PROJECT, 'utils'), path.join(PATH_TO_PROJECT, 'pylint.rc')
    )
...
def run_pytests_integral_tests(session):
    command = 'pytest -vs {} -c {} -m test_integrals'.format(PATH_TO_PROJECT, path.join(PATH_TO_PROJECT, 'pytest.ini'))
    session.install('-r', path.join(PATH_TO_PROJECT, 'requirements.txt'))
    session.run(*command.split(), external=True)

@nox.session(python=PYTHON_INTERP_VERSION)
def run_pytests_derivative_tests(session):
    command = 'pytest -vs {} -c {} -m test_derivatives'.format(PATH_TO_PROJECT, path.join(PATH_TO_PROJECT, 'pytest.ini'))
    session.install('-r', path.join(PATH_TO_PROJECT, 'requirements.txt'))
    session.run(*command.split(), external=True)
